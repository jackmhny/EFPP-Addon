package efpp.addon.modules;

import baritone.api.BaritoneAPI;
import baritone.api.pathing.goals.GoalBlock;
import efpp.addon.EFPPAddon;
...
import static efpp.addon.util.Utils.*;
...
public class ElytraFlyPlusPlus extends Module {
    ...
    private void clientJump() {
        float f = ((efpp.addon.mixin.accessor.LivingEntityAccessor) mc.player).invokeGetJumpVelocity();
        if (!(f <= 1.0E-5F)) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                for (int y = targetY.get(); y < targetY.get() + 3; y++) {
                    ...
                    if (distancePointToDirection(Vec3d.of(position), moveDir, mc.player.getEntityPos()) > portalScanWidth.get())
                        continue;
